...
const forEachTwoToThe = (arr) => {arr.forEach((element, index) => {arr[index] = Math.pow(2, element)}); return arr;}
...
const evenOdd = (arr) => arr.map(element => !isNaN(element) ? (element%2 ? 'odd' : 'even') : 'N/A');
...
const extractStats = (arr) => arr.map(element => ({'name': element.stat.name,'total': parseInt(element.baseStat) + parseInt(element.effort)}))
