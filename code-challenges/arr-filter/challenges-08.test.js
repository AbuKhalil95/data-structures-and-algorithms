...
const oddValues = (arr) => arr.filter(element => element%2 ? element : 0);
...
const filterStringsWithVowels = (arr) => arr.filter(element => {
  let checkVowel = false;
  let arrLetters = element.split('');
  arrLetters.forEach(s => {
    if (['a', 'e', 'i', 'o', 'u'].indexOf(s.toLowerCase()) >= 0) {
      checkVowel = true;
    }
  });
  return checkVowel;
...
const notInFirstArray = (forbiddenValues, arr) => arr.filter(element => forbiddenValues.includes(element) ? 0 : element);
...
const getBaseStatGreaterThan = (arr, minBaseStat) => arr.filter( element => element.baseStat > minBaseStat ? element : 0 );
...
const getCharactersWithoutChildren = (arr) => arr.filter(element => element.children ? 0 : element);
...
const evenOddNumericValues = (arr) => {
  let newArr = arr.filter(element => element === `${element}` ? 0 : element);
