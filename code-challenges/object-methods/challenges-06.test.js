...
const templatingWithMustache = () => {
  ...
  Object.keys(characters).forEach(element => {
    templateArray.push(Mustache.render(template, characters[element]));
  });
...
const hasChildrenEntries = (arr, character) => {
  let hasChildren = false;
  Object.entries(arr).forEach(element => element[1].name == character ? element[1].children.length ? hasChildren = true : 0 : 0);
  return hasChildren;};
  ...
  Object.values(arr).forEach(element => {
    ...
    if (element.spouse) {
      ...
      if (element.children.length) {
        totalCharac += element.children.length;
      }
    }
  });
  return totalCharac;
...
const houseSize = (arr) => {
  const sizes = [];
  Object.values(arr).forEach(element => {
    let totalCharHouse = 0;
    totalCharHouse++;
    if (element.spouse) {
      totalCharHouse++;
      if (element.children.length) {
        totalCharHouse += element.children.length;
      }
    }
    sizes.push({'house': element.house, 'members': totalCharHouse});
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  Object.values(arr).forEach(element => {
    let totalCharHouse = 0;
    totalCharHouse++;
    if (!deceasedSpouses.includes(element.spouse) && element.spouse != null) {
      totalCharHouse++;
    }
    if (element.children.length) {
      totalCharHouse += element.children.length;
    }
    survivors.push({'house': element.house, 'members': totalCharHouse});
  });
  return survivors;
};
